...
export interface AreNodesAdjacentFn {
  (path: DrawnPath, nodeId1: string, nodeId2: string): boolean;
}

export interface ComputeMidpointNodeFn {
  (node1: Node, node2: Node): Node;
}

export interface InsertSubdivisionNodeFn {
  (path: DrawnPath, nodeId1: string, nodeId2: string): DrawnPath;
}
 ...
 */
export const areNodesAdjacent: AreNodesAdjacentFn = (
  path,
  nodeId1,
  nodeId2,
) => {
 ...
 */
export const computeMidpointNode: ComputeMidpointNodeFn = (node1, node2) => ({
 ...
 */
export const insertSubdivisionNode: InsertSubdivisionNodeFn = (
  path,
  nodeId1,
  nodeId2,
) => {
